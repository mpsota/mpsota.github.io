...
module(require,stream.Readable);require.prototype._read=function(){var resolve=this._resumeFetch;resolve&&(this._resumeFetch=null,resolve())};require.prototype._onXHRProgress=function(){var self=this,xhr=self._xhr,response=null;switch(self._mode){case "text:vbarray":if(xhr.readyState!==rStates.DONE)break;try{response=(new global.VBArray(xhr.responseBody)).toArray()}catch(e){}if(null!==response){self.push(new Buffer(response));break}case "text":try{response=xhr.responseText}catch(e$16){self._mode=
